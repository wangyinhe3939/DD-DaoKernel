...
![Skills](https://img.shields.io/badge/Skills-8-2563EB?style=for-the-badge)
![Agents](https://img.shields.io/badge/Agents-4-7C3AED?style=for-the-badge)
...
把 DD-DaoKernel 放入 Codex 项目后，从总控开始：
...
<details>
<summary><b>为什么“证”必须存在？</b></summary>

古代哲学可以假定“圣人知之”，现代 Agent 不行。

模型会补全、推断、合理化。所以我主动加了一道现代护栏：
...
先保护原始数据、生产环境、权限、密钥、用户资产、回滚能力和退出路线。
...
再问：它最可能死在哪里？哪个优点后来变成缺点？哪一步其实早就该停？
...
判断本质、选择 Skill、收敛任务、决定下一步。

**单任务最多调用 3 个 Skills。**

优点是敢删任务；缺点也是敢删任务——它可能过早收敛。

### ◉ DD-观道参谋 · READ

只读观察事实、盲点、隐藏假设、反事实、利益关系和二阶后果。
...
`smallest patch · reversible · verifiable · no side quest`
...
`改一个颜色 · 改一句文案 · 明确的一行 Bug · 格式化文件`

直接做。

如果每个小任务都启动全部内核，那不是大道。
...
It does not primarily add capability. It constrains capability.
